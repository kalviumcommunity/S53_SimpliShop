...
async function compareProducts(req, res) {
    try {
        ...
        if (!productA || !productB) {
            return res.status(400).json({ error: "Provide productA and productB" });
        }

        const foundA = products.find(p => p.name.toLowerCase().includes(productA.toLowerCase()));
        const foundB = products.find(p => p.name.toLowerCase().includes(productB.toLowerCase()));
